...
```properties
post.processor.chain=at.grahsl.kafka.connect.processor.field.renaming.RenameByMapping
```
...
```properties
document.id.strategy=at.bronzels.kafka.connect.mongodb.strategy.BsonOidStrategy
```
...
* **default behaviour** at.bronzels.kafka.connect.mongodb.writemodel.strategy.**ReplaceOneDefaultStrategy**
* **business key** (-> see [use case 1](https://github.com/hpgrahsl/kafka-connect-mongodb#use-case-1-employing-business-keys)) at.grahsl.kafka.connect.writemodel.strategy.**ReplaceOneBusinessKeyStrategy**
* **delete on null values** at.grahsl.kafka.connect.writemodel.strategy.**DeleteOneDefaultStrategy** implicitly used when config option _mongodb.delete.on.null.values=true_ for [convention-based deletion](https://github.com/hpgrahsl/kafka-connect-mongodb#convention-based-deletion-on-null-values)
* **add inserted/modified timestamps** (-> see [use case 2](https://github.com/hpgrahsl/kafka-connect-mongodb#use-case-2-add-inserted-and-modified-timestamps)) at.grahsl.kafka.connect.writemodel.strategy.**UpdateOneTimestampsStrategy**
...
```json
{
  ...
  "config": {
    ...
    "document.id.strategy": aat.grahsl.kafka.connect.processorngodb.key.projection.list": "fieldA,fieldB",
    "key.projection.type": "whitelist",
    "writemodel.strategy": aat.grahsl.kafka.connect.mongodb.writemodel.strategy.ReplaceOneBusinessKeyStrategy`
...
```json
{
  ...
  "config": {
    ...
    "document.id.strategy": "at.bronzels.kafka.connect.mongodb.processor.iat.grahsl.kafka.connect.processor"mongodb.writemodel.strategy": "at.grahsl.at.bronzels.kafka.connect.mongodb.writemodel.strategy.UpdateOneTimestampsStrategy`
...
```json
{
  ...
  "config": {
    ...
    "value.converter":"io.confluent.connect.avro.AvroConverter",
    "value.converter.schema.registry.url":"http://localhost:8081",
  	"connector.class": "at.bronzels.kafka.connect.mongodb.MongoDbSinkConnector",
    "topics": "myreplset.kafkaconnect.mongosrc",
    "connection.uri": "mongodb://mongodb:27017/kafkaconnect?w=1&journal=true",
    "change.data.capture.handler": at.grahsl.kafka.connect.cdcongodb.collection": "mongosink"
  }
}
```
...
|-------------------------------------|------------------------------------------------------------------------------------------------------|---------|-------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------|------------|
| collection                  | single sink collection name to write to                                                              | string  | ""                                                                            |                                                                                                                  | high       |
| connection.uri              | the monogdb connection URI as supported by the offical drivers                                       | string  | mongodb://localhost:27017/kafkaconnect?w=1&journal=true                       |                                                                                                                  | high       |
| document.id.strategy        | class name of strategy to use for generating a unique document id (_id)                              | string  | at.bronzels.kafka.connect.mongodb.processor.iat.grahsl.kafka.connect.processor                                                                                                             | high       |
...
| max.batch.size              | maximum number of sink records to possibly batch together for processing                             | int     | 0                                                                             | [0,...]                                                                                                          | medium     |
...
| change.data.capture.handler | class name of CDC handler to use for processing                                                      | string  | ""                                                                            |                                                                                                                  | low        |
| document.id.strategies      | comma separated list of custom strategy classes to register for usage                                | string  | ""                                                                            |                                                                                                                  | low        |
| field.renamer.mapping       | inline JSON array with objects describing field name mappings                                        | string  | []                                                                            |                                                                                                                  | low        |
| field.renamer.regexp        | inline JSON array with objects describing regexp settings                                            | string  | []                                                                            |                                                                                                                  | low        |
| key.projection.list         | comma separated list of field names for key projection                                               | string  | ""                                                                            |                                                                                                                  | low        |
| key.projection.type         | whether or not and which key projection to use                                                       | string  | none                                                                          | [none, blacklist, whitelist]                                                                                     | low        |
| post.processor.chain        | comma separated list of post processor classes to build the chain with                               | string  | at.bronzels.kafka.connect.mongodb.processor.Dat.grahsl.kafka.connect.processor                                                                                                             | low        |
...
| value.projection.list       | comma separated list of field names for value projection                                             | string  | ""                                                                            |                                                                                                                  | low        |
| value.projection.type       | whether or not and which value projection to use                                                     | string  | none                                                                          | [none, blacklist, whitelist]                                                                                     | low        |
| mongodb.writemodel.strategy         | how to build the write models for the sink documents                                                 | string  | at.grahsl.at.bronzels.kafka.connect.mongodb.writemodel.strategy.ReplaceOneDefaultStrategy                                                                                                          | low        |
...
collection.foo-t=foo-c
collection.blah-t=blah-c
...
* if the configuration property for ```collection``` is set to any non-empty string this MongoDB collection name will be taken for any Kafka topic for which there is no defined mapping
...
document.id.strategy.foo-c=at.bronzels.kafka.connect.mongodb.processor.iat.grahsl.kafka.connect.processoressor.chain.foo-c=at.bronzels.kafka.connect.mongodb.processor.Dat.grahsl.kafka.connect.processorngodb.processor.Bat.grahsl.kafka.connect.processorection.type.foo-c=blacklist
value.projection.list.foo-c=k2,k4 
max.batch.size.foo-c=100
...
document.id.strategy.blah-c=at.bronzels.kafka.connect.mongodb.processor.iat.grahsl.kafka.connect.processorb.post.processor.chain.blah-c=at.bronzels.kafka.connect.mongodb.processor.Wat.grahsl.kafka.connect.processorection.type.blah-c=whitelist
value.projection.list.blah-c=k3,k5 
mongodb.writemodel.strategy.blah-c=at.grahsl.kafka.connect.writemodel.strategy.UpdateOneTimestampsStrategy
...
document.id.strategy=at.bronzels.kafka.connect.mongodb.processor.iat.grahsl.kafka.connect.processorspecific overriding for document identity
document.id.strategy.foo-c=at.bronzels.kafka.connect.mongodb.processor.iat.grahsl.kafka.connect.processorcific overriding for write model
writemodel.strategy.blah-c=at.grahsl.kafka.connect.writemodel.strategy.UpdateOneTimestampsStrategy
